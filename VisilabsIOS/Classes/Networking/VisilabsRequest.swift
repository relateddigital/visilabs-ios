...
class VisilabsRequest {
    ...
    class func sendEventRequest(visilabsEndpoint: VisilabsEndpoint, properties: [String: String], headers: [String: String], timeoutInterval: TimeInterval, completion: @escaping ([String: String]?) -> Void) {
        ...
        let resource = VisilabsNetwork.buildResource(endPoint: visilabsEndpoint, method: .get, timeoutInterval: timeoutInterval, requestBody: nil, queryItems: queryItems, headers: headers, parse: {_ in return true})
        ...
        VisilabsNetwork.apiRequest(resource: resource,
            failure: { (error, _, response) in
                ...
                if let httpResponse = response as? HTTPURLResponse, let url = httpResponse.url {
                    ...
                    VisilabsLogger.error("\(VisilabsBasePath.getEndpoint(visilabsEndpoint: resource.endPoint)) can not convert to HTTPURLResponse")
                    completion(nil)
                }
